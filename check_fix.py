...
class FixCheck(BaseCheck):
    ...
    def perform_check(self, menu_key):
        ...
        elif menu_key == 'check_fix_book_paths_cases':
            self.check_paths_case_match()
    ...
    def check_paths_case_match(self):
        ...
        def evaluate_book(book_id, db):
            ...
            if not os.path.exists(os.path.join(library_path,new_path).replace('/',os.sep)):
                return False
            actual_path = os.path.relpath(Path(os.path.join(library_path, new_path)).resolve(), library_path).replace(os.sep,'/')

            if new_path == actual_path:
                return False
            
            self.log('Altering path case to match DB: %s => %s' % (actual_path, new_path))
            print('Altering path case to match DB: %s => %s' % (actual_path, new_path))
            mod_path = '/'.join(['_'+p for p in actual_path.split('/')])
            final_actual_path = os.path.join(library_path, actual_path).replace('/',os.sep)
            final_mod_path = os.path.join(library_path, mod_path).replace('/',os.sep)
            final_new_path = os.path.join(library_path, new_path).replace('/',os.sep)
            final_actual_dir = os.path.join(library_path, actual_path.split('/')[0]).replace('/',os.sep)
            final_mod_dir = os.path.join(library_path, mod_path.split('/')[0]).replace('/',os.sep)
            final_new_dir = os.path.join(library_path, new_path.split('/')[0]).replace('/',os.sep)
            print("Actual Path: %s" % final_actual_path)
            print("Mod Path: %s" % final_mod_path)
            print("New Path: %s" % final_new_path)
            print("Actual Dir: %s" % final_actual_dir)
            print("Mod Dir: %s" % final_mod_dir)
            print("New Dir: %s" % final_new_dir)
            print("========================\n\n")

            actual_paths.append(final_actual_path)
            mod_paths.append(final_mod_path)
            new_paths.append(final_new_path)
            if not final_actual_dir in actual_dirs:
                actual_dirs.append(final_actual_dir)
                mod_dirs.append(final_mod_dir)
                new_dirs.append(final_new_dir)
            return True


        total_count, result_ids, cancelled_msg = self.check_all_files(evaluate_book,
                                                                  marked_text='book_path_case_updated',
                                                                  status_msg_type='books with paths missing commas')
        print("Moving actual files to mod files")
        for j in range(0,len(actual_paths)):
            shutil.move(actual_paths[j],mod_paths[j])
        print("Moving actual dirs to mod dirs")
        for j in range(0,len(actual_dirs)):
            for file_name in os.listdir(actual_dirs[j]):
                shutil.move(os.path.join(actual_dirs[j], file_name), os.path.join(mod_dirs[j], file_name))
            shutil.rmtree(actual_dirs[j])
        print("Moving mod files to new files")
        for j in range(0,len(actual_paths)):
            shutil.move(mod_paths[j],new_paths[j])
        print("Moving mod dirs to new dirs")
        for j in range(0,len(actual_dirs)):
            for file_name in os.listdir(mod_dirs[j]):
                shutil.move(os.path.join(mod_dirs[j], file_name), os.path.join(new_dirs[j], file_name))
            shutil.rmtree(mod_dirs[j])

        msg = 'Checked %d books, updated %d paths cases%s' % \
                    (total_count, len(result_ids), cancelled_msg)
        self.gui.status_bar.showMessage(msg)
        if len(result_ids) == 0:
            return info_dialog(self.gui, 'No Matches%s'%cancelled_msg, 'All books have up to date paths cases', show=True)
        self.gui.library_view.model().refresh_ids(list(result_ids))
